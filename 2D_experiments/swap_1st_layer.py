...
if no_siren_only_mlp:
    model_cur = FFN(*network_size).cuda()
    model_next = FFN(*network_size).cuda()
    model_mix = FFN(*network_size).cuda()
else:
    model_cur = SIRENFF(*network_size).cuda()
...
if do_color_scheme_transfer:
    ...
    # quantized_result_dir = 'imgs_swap_1st_quant_delta'
...
model_fn_cur = os.path.join(model_dir, f'{base_frame:04d}.pth')
...
for frame_i, frame in enumerate(data_iterator):
    ...
    if do_color_scheme_transfer:
        if frame_i == (base_frame + 1):
            model_mix.load_state_dict(torch.load(model_fn_cur))
    else:
        model_mix.load_state_dict(torch.load(model_fn_cur))
        if no_siren_only_mlp:
    ...
    # make query coordinates
    with torch.no_grad():
        out_cur, _ = model_cur(input_mapping(test_data[0], B, use_nerf_pe=use_nerf_pe))
    ...
    # if not do_color_scheme_transfer:
