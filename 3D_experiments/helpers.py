...
def batchify_rays(rays_flat, chunk=1024*32, **kwargs):
    ...
    for i in range(0, rays_flat.shape[0], chunk):
        ...
        # ret = render_rays_nerfacc(rays_flat[i:i + chunk], **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def batchify_rays_blend_models(rays_flat, chunk=1024*32, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays_blend_models(rays_flat[i:i+chunk], **kwargs)
        # ret = render_rays_nerfacc(rays_flat[i:i + chunk], **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render_blend_two_models(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = batchify_rays_blend_models(rays, chunk, **kwargs)
    for k in all_ret:
        k_sh = list(sh[:-1]) + list(all_ret[k].shape[1:])
        all_ret[k] = torch.reshape(all_ret[k], k_sh)
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None,
                render_factor=0, save_name=None):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        # r = Rotation.from_matrix(render_poses[i,:3,:3].detach().cpu())
        ...
        if savedir is not None:
            ...
            if save_name is None:
                ...
                filename = os.path.join(savedir, f'{save_name}.png')
            imageio.imwrite(filename, rgb8)

            disp8 = to8b(disps[-1] / np.max(disps[0]))
            if save_name is None:
                filename = os.path.join(savedir, 'depth_{:03d}.png'.format(i))
            else:
                filename = os.path.join(savedir, f'depth_{save_name}.png')
...
def create_nerf(args):
    ...
    if (args.dataset_type != 'llff' and args.dataset_type != 'little_falls'
        and args.dataset_type != 'META')  or args.no_ndc:
        print('Not ndc!')
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, ckpt_path
...
def raw2outputs_blend_two_models(raw_s, raw_d, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    w_rgb = F.softmax(torch.stack([rgb_s, rgb_d],dim=0),dim=0)
    rgb = w_rgb[0] * rgb_s + w_rgb[1] * rgb_d
    noise = 0.
    if raw_noise_std > 0.:
    ...
    alpha = raw2alpha(raw_s[..., 3] + raw_d[..., 3] + noise, dists)  # [N_rays, N_samples]
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1. - alpha + 1e-5], -1), -1)[:,:-1]
    rgb_map = torch.sum(weights[...,None] * rgb, -2)  # [N_rays, 3]
...
#     raw = run_network(pts)
    ...
    if N_importance > 0:
        ...
        rgb_map_0, disp_map_0, acc_map_0, alpha_map0, weights0 = rgb_map, disp_map, acc_map, alpha_map, weights
...
def render_rays_blend_models(ray_batch,
                network_fn_s, network_fine_s, network_fn_d, network_fine_d,
                network_query_fn,
                N_samples,
                retraw=False,
                lindisp=False,
...
def create_ray_rgb_K_batches(poses, H, W, K, images, i_train, masks=None):
    # For random ray batching
    print('get rays')
    rays = []
    for i in range(poses.shape[0]):
        p = poses[i, :3, :4]
        rays.append(get_rays_np(H, W, K[i], p))
    rays = np.stack(rays, 0)  # [N, ro+rd, H, W, 3]
    print('done, concats')
    # [N, ro+rd+rgb+K[0:3], H, W, 3]
    K_tmp = np.tile(K[:, :, None, None], (1, 1, H, W, 1))
    rays_rgb = np.concatenate([rays, images[:, None], K_tmp], 1)
    rays_rgb = np.transpose(rays_rgb, [0, 2, 3, 1, 4])  # [N, H, W, ro+rd+rgb+K[0:3], 3]
    # rays_rgb = np.stack([rays_rgb[i] for i in i_train], 0)  # train images only
    rays_rgb = rays_rgb[i_train]
    rays_rgb = np.reshape(rays_rgb, [-1, 3 + 3, 3])  # [(N-1)*H*W, ro+rd+rgb+K[0:3], 3]
    rays_rgb = rays_rgb.astype(np.float32)
    print('shuffle rays')
    # np.random.shuffle(rays_rgb)
    # permuted_i = np.random.permutation(rays_rgb.shape[0])
    permuted_i = torch.randperm(rays_rgb.shape[0]).cuda()
    rays_rgb = torch.tensor(rays_rgb).cuda()[permuted_i].cpu().numpy()
    if masks is not None:
        masks = np.transpose(masks[:, None], [0, 2, 3, 1, 4])  # [N, H, W, 1, 3]
        # masks = np.stack([masks[i] for i in i_train], 0)  # train images only
        masks = masks[i_train]  # train images only
        masks = np.reshape(masks, [-1, 1, 3])  # [(N-1)*H*W, 1, 3]
        # masks = masks.astype(np.float32)
        masks = torch.tensor(masks).cuda()[permuted_i].cpu().numpy()

        # rgb_prev = rgb_prev[:, None].permute(0, 2, 3, 1, 4)  # [N, H, W, 1, 3]
        # rgb_prev = rgb_prev[i_train]  # train images only
        # rgb_prev = rgb_prev.view(-1, 1, 3)  # [(N-1)*H*W, 1, 3]
        # rgb_prev = rgb_prev.float()
        # rgb_prev = rgb_prev[permuted_i]

        permuted_i = permuted_i.cpu().numpy()
        return rays_rgb, permuted_i, masks
    else:
        permuted_i = permuted_i.cpu().numpy()
        return rays_rgb, permuted_i, None

def cam_rays(H, W, K, c2w, ndc=True):
    """ create rays for camera and split them in two sets
    Args:
      H: int. Height of image in pixels.
      W: int. Width of image in pixels.
      K camera intrisics: float. Focal length of pinhole camera.
      c2w: array of shape [3, 4]. Camera-to-world transformation matrix.
      ndc: bool. If True, represent ray origin, direction in NDC coordinates.
    Returns:
      rays_o, rays_d - flattened rays.
    """
    rays_o, rays_d = get_rays(H, W, K, c2w)
